        ...
        <div class="main-content flex-1 bg-gray-100 mt-12 md:mt-2 pb-24 md:pb-5">
            ...
            <div class="my-5 pb-5 mx-auto w-full px-6 ">
                <div class="bg-white text-gray-900 font-bold border-1 border rounded shadow-xl ">
                    <div class="pt-5 px-6 flex space-x-8 mx-auto justify-center pb-5 w-9/12 border-b-2">
                    ...
                    <div class="p-4 flex space-x-12">
                        <h1 class="flex-auto flex my-auto text-xl justify-start">
                            Student's Registered Course (<?=$pendingCount?>)
                        </h1>
                        <div class="flex">
                            <a class="-mr-3" href="paymentSuccess.php"><button class="text-sm text-right font-semibold text-indigo-500 border border-indigo-500 h-10 px-5 m-2 rounded bg-gray-100 hover:bg-indigo-200 focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:ring-opacity-50">
                                Succesfull Payment ( <?=$successCount?> )
                            </button></a>
                            <a href="paymentRefund.php"><button class="text-sm font-semibold text-indigo-500 border border-indigo-500 h-10 px-5 m-2 rounded bg-gray-100 hover:bg-indigo-200 focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:ring-opacity-50">
                                Cancelled Payment
                            </button></a>
                        </div>
                    </div>
                         ...
                         <table class="w-full text-md bg-white shadow-md rounded mb-4 border border-2 table-fixed">
                             <thead>
                                <tr class="border-b text-sm">
                                    ...
                                    <th class="text-center py-3 border-r border-gray-200" width="150px">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                $i=1;
                                foreach($pending as $roww){
                                    if ($pendingCount > 0){
                                        if($roww['paymentStatus'] == 1){
                                            $status = "PENDING";
                                        }
                                        else{
                                            $status = "INACTIVE";
                                        ...
                                        echo "<tr class='border-b hover:bg-blue-100 bg-gray-100'>"
                                            ...
                                            . "<td width='100px' class='cursor-default text-sm text-center font-normal border-r border-gray-200'><h class='font-bold'>PAID</h1></td>"
                                            . "<td class='cursor-default text-center flex space-y-2 justify-center'><form action='' method='POST'>"
                                            . "<input type='hidden' name='userT' value='".$roww['tutorUsername']."'>"
                                            . "<input type='hidden' name='userS' value='".$roww['studentUsername']."'>"
                                            . "<input type='hidden' name='course' value='".$roww['courseFull']."'>"
                                            . "<input type='hidden' name='refno' value='".$roww['paymentID']."'>";
                                            /*if($status == "ACTIVE"){
                                            ...
                                            echo "&nbsp;&nbsp;<input type='submit' name='paid' 
                                                    class='cursor-pointer text-xs h-9 px-5 my-1 text-blue-100 transition-colors duration-150 bg-blue-600 rounded-lg focus:shadow-outline hover:bg-blue-700' 
                                                    value='PAID TUTOR'>
                                                    &nbsp;<input type='submit' name='refund' 
                                                    class='cursor-pointer text-xs h-9 px-5 my-1 text-red-100 transition-colors duration-150 bg-red-700 rounded-lg focus:shadow-outline hover:bg-red-800' 
                                                    value='REFUND'>
                                                    </form></td>
                                                    </tr>";
                                        $i++;
