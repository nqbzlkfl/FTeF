<!-- This is an example component -->
<div class="h-auto">
 
 <link rel="stylesheet" href="https://unpkg.com/tailwindcss@^1.0/dist/tailwind.min.css">
 <script src="https://cdn.jsdelivr.net/gh/alpinejs/alpine@v2.x.x/dist/alpine.js" defer></script>

 <style>
     [x-cloak] {
         display: none;
     }

     [type="checkbox"] {
         box-sizing: border-box;
         padding: 0;
     }

     .form-checkbox,
     .form-radio {
         -webkit-appearance: none;
         -moz-appearance: none;
         appearance: none;
         -webkit-print-color-adjust: exact;
         color-adjust: exact;
         display: inline-block;
         vertical-align: middle;
         background-origin: border-box;
         -webkit-user-select: none;
         -moz-user-select: none;
         -ms-user-select: none;
         user-select: none;
         flex-shrink: 0;
         color: currentColor;
         background-color: #fff;
         border-color: #e2e8f0;
         border-width: 1px;
         height: 1.4em;
         width: 1.4em;
     }

     .form-checkbox {
         border-radius: 0.25rem;
     }

     .form-radio {
         border-radius: 50%;
     }

     .form-checkbox:checked {
         background-image: url("data:image/svg+xml,%3csvg viewBox='0 0 16 16' fill='white' xmlns='http://www.w3.org/2000/svg'%3e%3cpath d='M5.707 7.293a1 1 0 0 0-1.414 1.414l2 2a1 1 0 0 0 1.414 0l4-4a1 1 0 0 0-1.414-1.414L7 8.586 5.707 7.293z'/%3e%3c/svg%3e");
         border-color: transparent;
         background-color: currentColor;
         background-size: 100% 100%;
         background-position: center;
         background-repeat: no-repeat;
     }
     
     .form-radio:checked {
         background-image: url("data:image/svg+xml,%3csvg viewBox='0 0 16 16' fill='white' xmlns='http://www.w3.org/2000/svg'%3e%3ccircle cx='8' cy='8' r='3'/%3e%3c/svg%3e");
         border-color: transparent;
         background-color: currentColor;
         background-size: 100% 100%;
         background-position: center;
         background-repeat: no-repeat;
     }
 </style>

 <div x-data="app()" x-cloak>
     <div class="max-w-3xl mx-auto px-4 py-8 h-auto">

         <div x-show.transition="step === 'complete'">
             <div class="bg-white rounded-lg p-10 flex items-center shadow justify-between">
                 <div>
                     <svg class="mb-4 h-20 w-20 text-green-500 mx-auto" viewBox="0 0 20 20" fill="currentColor">  <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>

                     <h2 class="text-2xl mb-4 text-gray-800 text-center font-bold">Registration Success</h2>

                     <div class="text-gray-600 mb-8">
                         Thank you. We have sent you an email to demo@demo.test. Please click the link in the message to activate your account.
                     </div>

                     <button
                         @click="step = 1"
                         class="w-40 block mx-auto focus:outline-none py-2 px-5 rounded-lg shadow-sm text-center text-gray-600 bg-white hover:bg-gray-100 font-medium border" 
                     >Back to home</button>
                 </div>
             </div>
         </div>

         <div x-show.transition="step != 'complete'">	
             <!-- Top Navigation -->
             <div class="border-b-2 py-4">
                 <div class="flex-inline flex">
                    <div class="flex-1 uppercase tracking-wide text-xs font-bold text-gray-500 mb-1 leading-tight" x-text="`Step: ${step} of 5`"></div>
                    
                    <div class="flex items-center md:w-64">
                            <div class="w-full bg-green-200 rounded-full mr-2">
                                <div class="rounded-full bg-green-500 text-xs leading-none h-2 text-center text-white" :style="'width: '+ parseInt(step / 5 * 100) +'%'"></div>
                            </div>
                            <div class="text-xs w-10 text-gray-600" x-text="parseInt(step / 5 * 100) +'%'"></div>
                    </div>
                 </div>
                 
                 <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                     <div class="flex-1">
                         <div x-show="step === 1">
                             <div class="pt-2 text-lg font-bold text-gray-700 leading-tight">Setup Your Account 1/2</div>
                         </div>
                         
                         <div x-show="step === 2">
                             <div class="pt-2 text-lg font-bold text-gray-700 leading-tight">Setup Your Account 2/2</div>
                         </div>

                         <div x-show="step === 3">
                             <div class="pt-2 text-lg font-bold text-gray-700 leading-tight">You Personal Details</div>
                         </div>
                         <div x-show="step === 4">
                             <div class="pt-2 text-lg font-bold text-gray-700 leading-tight">You Studies Details</div>
                         </div>
                         <div x-show="step === 5">
                             <div class="pt-2 text-lg font-bold text-gray-700 leading-tight">Please Choose a Subject to Teach</div>
                         </div>
                     </div>
                     
                 </div>
             </div>
             <!-- /Top Navigation -->

             <!-- Step Content -->
             <div class="pb-14 pt-6 static">	
                 <div x-show.transition.in="step === 1">

                     <div class="mb-6 pt-3 rounded bg-gray-200">
                         <label for="email" class="block text-gray-700 text-sm font-bold mb-1 ml-3">Email</label>
                         <input type="email" name="email" id="email" required
                             class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-blue-600 transition duration-500 px-3 pb-3"
                             placeholder="Enter your email...">
                     </div>
                     <div class="mb-6 pt-3 rounded bg-gray-200">
                         <label for="username" class="block text-gray-700 text-sm font-bold mb-1 ml-3">Username</label>
                         <input type="text" name="username" id="username" required
                             class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-blue-600 transition duration-500 px-3 pb-3"
                             placeholder="Enter your username...">
                     </div>
                     <div class="mb-6 pt-3 rounded bg-gray-200">
                         <label for="password" class="block text-gray-700 text-sm font-bold mb-1 ml-3">Password</label>
                         <input type="password" name="password" id="password" required
                             class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-blue-600 transition duration-500 px-3 pb-3"
                             placeholder="Enter your password...">
                     </div>
                 </div>
                 <div x-show.transition.in="step === 2">
    
                    <div class="mb-5 text-center">
                         <div class="mx-auto w-32 h-32 mb-2 border rounded-full relative bg-gray-100 mb-4 shadow-inset">
                             <img id="image" class="object-cover w-full h-32 rounded-full" :src="image" />
                         </div>
                         
                         <label 
                             for="fileInput"
                             type="button"
                             class="cursor-pointer inine-flex justify-between items-center focus:outline-none border py-2 px-4 rounded-lg shadow-sm text-left text-gray-600 bg-white hover:bg-gray-100 font-medium"
                         >
                             <svg xmlns="http://www.w3.org/2000/svg" class="inline-flex flex-shrink-0 w-6 h-6 -mt-1 mr-1" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                 <rect x="0" y="0" width="24" height="24" stroke="none"></rect>
                                 <path d="M5 7h1a2 2 0 0 0 2 -2a1 1 0 0 1 1 -1h6a1 1 0 0 1 1 1a2 2 0 0 0 2 2h1a2 2 0 0 1 2 2v9a2 2 0 0 1 -2 2h-14a2 2 0 0 1 -2 -2v-9a2 2 0 0 1 2 -2" />
                                 <circle cx="12" cy="13" r="3" />
                             </svg>						
                             Browse Photo
                         </label>

                         <div class="mx-auto w-48 text-gray-500 text-xs text-center mt-1">You can add or skip profile picture later</div>

                         <input name="tutorPhoto" id="fileInput" accept="image/*" class="hidden" type="file" @change="let file = document.getElementById('fileInput').files[0]; 
                             var reader = new FileReader();
                             reader.onload = (e) => image = e.target.result;
                             reader.readAsDataURL(file);">
                    </div>
                     <div class="mb-6 pt-3 rounded bg-gray-200">
                         <label for="fname" class="block text-gray-700 text-sm font-bold mb-1 ml-3">Full name</label>
                         <input type="text" name="fname" id="fname" required
                             class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-blue-600 transition duration-500 px-3 pb-3"
                             placeholder="Enter your full name...">
                     </div>
                 </div>

                 <div x-show.transition.in="step === 3">
                    
                     <div class="mb-6 pt-3 rounded bg-gray-200">
                         <label for="matricid" class="block text-gray-700 text-sm font-bold mb-1 ml-3">Matric ID</label>
                         <input type="text" name="matricid" id="matricid" required
                             class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-blue-600 transition duration-500 px-3 pb-3"
                             placeholder="Enter matric ID...">
                     </div>
                     <div class="mb-6 pt-3 rounded bg-gray-200">
                         <label for="phone" class="block text-gray-700 text-sm font-bold mb-1 ml-3">Phone Number</label>
                         <input type="tel" name="phone" id="phone" required
                             class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-blue-600 transition duration-500 px-3 pb-3"
                             placeholder="Enter phone number...">
                     </div>
                     <div class="mb-6 pt-3 rounded bg-gray-200">
                         <label for="bod" class="block text-gray-700 text-sm font-bold mb-1 ml-3">Birth of Date</label>
                         <input type="date" name="bod" id="bod" required
                             class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-blue-600 transition duration-500 px-3 pb-3"
                             placeholder="Enter birth of date...">
                     </div>
                     <div class="mb-6 pt-3 rounded">
                         <label for="gender" class="font-bold mb-1 text-gray-700 block">Gender</label>
                         
                         <div class="flex">
                             <label
                                 class="flex justify-start items-center text-truncate rounded-lg bg-gray-200 pl-4 pr-6 py-3 shadow-sm mr-4
                                 focus:outline-none border-b-4 border-gray-300 focus:border-blue-600 transition duration-500">
                                 <div class="text-teal-600 mr-3">
                                     <input type="radio" x-model="gender" name="gender" value="Male" class="form-radio focus:outline-none focus:shadow-outline" />
                                 </div>
                                 <div class="select-none text-gray-700">Male</div>
                             </label>

                             <label
                                 class="flex justify-start items-center text-truncate rounded-lg bg-gray-200 pl-4 pr-6 py-3 shadow-sm
                                 focus:outline-none border-b-4 border-gray-300 focus:border-blue-600 transition duration-500">
                                 <div class="text-teal-600 mr-3">
                                     <input type="radio" x-model="gender" name="gender" value="Female" class="form-radio focus:outline-none focus:shadow-outline" />
                                 </div>
                                 <div class="select-none text-gray-700">Female</div>
                             </label>
                         </div>
                     </div>
                </div>
                
                <div x-show.transition.in="step === 4">
                <div class="mb-6 pt-3 rounded bg-gray-200">
                         <label for="faculty" class="block text-gray-700 text-sm font-bold mb-1 ml-3">Faculty</label>
                         <select type="text" name="faculty" id="faculty"
                             class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-blue-600 transition duration-500 px-3 pb-3"
                             >
                             <option value="" selected>--Choose Your Faculty--</option>
                             <option value="Faculty of Computing">Faculty of Computing</option>
                             <option value="Faculty of Electrical & Electronic Engineering Technology">Faculty of Electrical & Electronic Engineering Technology</option>
                             <option value="Faculty of Chemical And Process Engineering Technology">Faculty of Chemical And Process Engineering Technology</option>
                         </select>
                     </div>
                     <div class="mb-6 pt-3 rounded bg-gray-200">
                         <label for="program" class="block text-gray-700 text-sm font-bold mb-1 ml-3">Program</label>
                         <select type="text" name="program" id="program"
                             class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-blue-600 transition duration-500 px-3 pb-3"
                             >
                             <option value="" selected>--Choose Your Program--</option>
                             <option value="Software Engineering">Software Engineering</option>
                             <option value="Computer Systems & Networking">Computer Systems & Networking</option>
                             <option value="Graphics & Multimedia Technology">Graphics & Multimedia Technology</option>
                             <option value="Diploma">Diploma</option>
                             <option value="From other faculty">From other faculty</option>
                         </select>
                     </div>
                     <div class="mb-6 pt-3 rounded bg-gray-200">
                         <label for="yos" class="block text-gray-700 text-sm font-bold mb-1 ml-3">Year of Study</label>
                         <select type="text" name="yos" id="yos"
                             class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-blue-600 transition duration-500 px-3 pb-3"
                            >
                             <option value="" selected>--Choose Your Current Year of Study--</option>
                             <option value="1" name="1">Year 1</option>
                             <option value="2" name="2">Year 2</option>
                             <option value="3" name="3">Year 3</option>
                             <option value="4" name="4">Year 4</option>
                             <option value="5" name="5">Year 5</option>
                             <option value="6" name="6">Year 6</option>
                         </select>
                     </div>
                    
                </div>

                 <div x-show.transition.in="step === 5">
                     <div class="mb-1 pt-3 rounded bg-gray-200">
                         <label for="course" class="block text-gray-700 text-sm font-bold mb-1 ml-3">Year of Study</label>
                         <select type="text" name="course" id="course"
                             class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-blue-600 transition duration-500 px-3 pb-3"
                            >
                            <option value="" selected>--Subject to Choose--</option>
                            <?php
                                $ii= 0;
                                foreach($dataa as $row){
                                    $ii++;
                                    echo 
                                     "<option value='". $row['courseCode'] . ' ' .$row['courseName'] ."'>" .$row['courseCode']. ' ' .$row['courseName'] ."</option>";
                                }
                            ?>
                         </select>
                     </div>
                     
                     <div class="text-sm text-gray-500 text-right pb-2">You can add two or more subjects later on.</div>
                     
                     <div class="mb-1 py-4 pt-3 rounded bg-gray-200 border-b-4 border-gray-300 focus:border-blue-600 transition duration-500">
                        <div class="my-2 text-center flex-inline flex">
                            <div class="flex-1">
                                <div class="flex-inline">
                                    <label 
                                        for="fileInput1"
                                        type="button"
                                        class="flex-1 cursor-pointer inline-flex justify-between items-center focus:outline-none border py-2 px-4 rounded-lg shadow-sm text-left text-gray-600 bg-white hover:bg-gray-100 font-medium"
                                    >
                                                            
                                        Upload Result
                                    </label>
                                    <input class="flex-1 mx-auto text-gray-700 text-md text-center cursor-default focus:outline-none bg-transparent"
                                        id="disp" type="text" value="File name" readonly />
                                </div>

                                <div class="mx-auto w-9/12 text-gray-500 text-xs text-justify pt-2">Please upload your examination result based on the semester that included the subject you applied in PDF format only, as the proof of eligibility.</div>
                            </div>
                            <input 
                            required name="tutorGrad" id="fileInput1" class="hidden" accept="application/pdf"  type="file">
                        </div>
                    </div>
                 </div>
             </div>
             <!-- / Step Content -->
             
         </div>
     </div>

     <!-- Bottom Navigation -->	
     <div class="mx-auto bottom-0 left-0 right-0 py-5 bg-transparent absolute" x-show="step != 'complete'">
         <div class="max-w-3xl mx-auto px-4">
             <div class="flex justify-between">
                 <div class="w-1/2  text-left">
                     <button type="button"
                         x-show="step > 1"
                         @click="step--"
                         class="w-32 focus:outline-none py-2 px-5 rounded-lg shadow-sm text-center text-gray-600 bg-white hover:bg-gray-100 font-medium border" 
                     >Previous</button>
                 </div>

                 
                 <div class="w-1/2  text-right">
                     <button type="button"
                         x-show="step < 5"
                         @click="step++"
                         class="w-32 focus:outline-none border border-transparent py-2 px-5 rounded-lg shadow-sm text-center text-white bg-blue-500 hover:bg-blue-600 font-medium" 
                     >Next</button>
                                <!--@click="step = 'complete'"-->
                     <button
                          type="submit" name="register" value="register"
                         x-show="step === 5"
                         class="w-32 focus:outline-none border border-transparent py-2 px-5 rounded-lg shadow-sm text-center text-white bg-blue-500 hover:bg-blue-600 font-medium" 
                     >Complete</button>
                 </div>
             </div>
         </div>
     </div>
     <!-- / Bottom Navigation https://placehold.co/300x300/e2e8f0/cccccc -->	
 </div>

 <script>
    
    document.getElementById('fileInput1').onchange = uploadOnChange;
    function uploadOnChange() {
    var filename = this.value;
    var lastIndex = filename.lastIndexOf("\\");
    if (lastIndex >= 0) {
        filename = filename.substring(lastIndex + 1);
    }
    document.getElementById('disp').value = filename;
    }

     function app() {
         return {
             step: 1, 
             passwordStrengthText: '',
             togglePassword: false,
            
             /* document: 'data:application/pdf;' */
             image: 'data:image/jpeg;base64,',
             password: '',
             gender: '',

             checkPasswordStrength() {
                 var strongRegex = new RegExp("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{8,})");
                 var mediumRegex = new RegExp("^(((?=.*[a-z])(?=.*[A-Z]))|((?=.*[a-z])(?=.*[0-9]))|((?=.*[A-Z])(?=.*[0-9])))(?=.{6,})");

                 let value = this.password;

                 if (strongRegex.test(value)) {
                     this.passwordStrengthText = "Strong password";
                 } else if(mediumRegex.test(value)) {
                     this.passwordStrengthText = "Could be stronger";
                 } else {
                     this.passwordStrengthText = "Too weak";
                 }
             }
         }
     }
 </script>
</body>
</html>